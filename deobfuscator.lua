...
loadstring = function(al, eu)
    ...
    if cI:match("^https?://") then
        ...
        at(string.format('local cxyrosixseven = loadstring(game:HttpGet("%s"))()', cI))
        return function()
            return ez
        end
    end
...
function q.dump_file(eN, eO)
    q.reset()
    az("-- cxyro dumper: https://discord.gg/WW8Qm77dhF skids will never be us")
    ...
    local eR =
        ...
        {LuraphContinue = function()
            end, script = script, game = game, workspace = workspace, CXYRO_CHECKINDEX = function(x, ba)
                local aF = x[ba]
                ...
                return aF
            end, CXYRO_GET = function(b5)
                return b5
            end, CXYRO_CALL = function(as, ...)
                return as(...)
            end, CXYRO_NAMECALL = function(eS, em, ...)
                return eS[em](eS, ...)
...
function q.dump_string(al, eO)
    q.reset()
    az("-- cxyro dumper: https://discord.gg/WW8Qm77dhF")
    aA()
    if al then
        al = I(al)
    end
